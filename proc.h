...
struct proc {
    ...
    int             boostsleft;     // Number of boost rounds remaining
    uint            sleepstart;     // Tick when process started sleeping
    uint            sleepuntil;     // Tick when process should wake up
    int             runticks;       // Number of ticks process has run
     // -------- Threading support --------
    // Thread-related fields
    int is_thread;                 // 1 if this process is actually a thread
    int tid;                       // Thread ID (same as pid or unique per thread)
    struct proc *mainthread;       // Points to main thread (parent process)
    char *ustack;                  // User stack pointer (thread's private stack)
