...
void dabort_handler(struct trapframe *r)
{
    ...
    if(proc && proc->pid > 0) {
        // Handle page fault - try to allocate memory
        if(allocuvm(proc->pgdir, fa & ~(PTE_SZ-1), (fa & ~(PTE_SZ-1)) + PTE_SZ) != 0) {
            return;  // Successfully handled page fault
        }
    }

    // If we get here, it's either not a page fault or allocation failed
    cprintf("data abort: instruction 0x%x, fault addr 0x%x, status 0x%x\n",
            r->pc, fa, dfs);
    
    proc->killed = 1;  // Kill process on unhandled fault
}
