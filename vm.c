...
int sys_kpt(void) {
    // Print kernel page table entries
    pde_t *pgdir = (pde_t*)&kpgdir;
    
    cprintf("Kernel page table:\n");
    cprintf("NUM_PTE:%d\n",NUM_PTE);
    // Use NUM_UPDE instead of NUM_PDE if that's what's defined
    for(int i = 0; i < NUM_PTE; i++) {  // Add bounds check
        if((pgdir[i] & PE_TYPES) == KPDE_TYPE) {
            uint pa = PT_ADDR(pgdir[i]);
            uint ap = (pgdir[i] >> 10) & 0x3;
            uint domain = (pgdir[i] >> 5) & 0xF;
            uint flags = pgdir[i] & 0xFFF;
            cprintf("  pde %d: pa 0x%x ap 0x%x domain 0x%x flags 0x%x\n",i, pa, ap, domain, flags);  
        }
    }
    cprintf("Kernel page table dump complete\n");  // Add completion message
    return 0;
}
